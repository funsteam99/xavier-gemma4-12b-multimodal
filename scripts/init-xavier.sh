...
MODEL_DIR="${MODEL_DIR:-/media/nvidia/sd/models/gemma-4-e4b-it}"
MODEL="${MODEL:-$MODEL_DIR/gemma-4-E4B-it-Q4_K_M.gguf}"
MMPROJ="${MMPROJ:-$MODEL_DIR/mmproj-gemma-4-E4B-it-Q8_0.gguf}"
CONSOLE_TARGET="${CONSOLE_TARGET:-/media/nvidia/sd/gemma4-e4b-console}"
BACKEND_PORT="${BACKEND_PORT:-18084}"
CONSOLE_PORT="${CONSOLE_PORT:-18090}"
BACKEND_LOG="${BACKEND_LOG:-/tmp/llama_gemma4_e4b.log}"
CONSOLE_LOG="${CONSOLE_LOG:-/tmp/gemma4_e4b_console.log}"
...
check() {
  echo "checking Xavier Gemma 4 E4B prerequisites"
...
start_backend() {
  ...
  for _ in $(seq 1 45); do
    if curl -fsS "http://127.0.0.1:$BACKEND_PORT/health" >/tmp/gemma4_e4b_health.json 2>/dev/null; then
      echo "backend healthy:"
      cat /tmp/gemma4_e4b_health.json
      echo
      return 0
    fi
    sleep 2
...
start_console() {
  ...
  PORT="$CONSOLE_PORT" GEMMA_BACKEND="http://127.0.0.1:$BACKEND_PORT" \
...
Commands:
  ...
  start-backend     Start/restart llama-server
  start-console     Install and start/restart the browser console
  start             Start/restart backend and console
  status            Show ports and health checks
